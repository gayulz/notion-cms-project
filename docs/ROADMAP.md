...
- [x] Notion 데이터베이스 생성
  ...
  - Summary (요약)
- [x] 샘플 데이터 3~5개 작성
- [x] 인티그레이션에 데이터베이스 접근 권한 부여

#### 1.4 타입 정의

- [x] `types/notion.ts` 생성
...
- [x] `npm run dev` 실행 시 에러 없이 서버 시작
- [x] Notion API로 데이터베이스 쿼리 성공
- [x] 샘플 데이터 3개 이상 확인
...
- [x] shadcn/ui 컴포넌트 설치
  ...
  ```
- [x] `PostCard` 컴포넌트 (`components/PostCard.tsx`)
  - 썸네일 이미지
  - 제목, 요약
  - 발행일, 카테고리, 태그
  - 링크
- [x] `PostMeta` 컴포넌트 (`components/PostMeta.tsx`)
  - 발행일, 카테고리, 태그 표시
- [x] `Layout` 컴포넌트 (`components/Layout.tsx`)
  - Header (로고, 네비게이션)
  - Footer
  - 공통 레이아웃 구조
...
- [x] Tailwind CSS 설정 커스터마이징
  - 색상 팔레트 정의
  - 타이포그래피 설정
  - 브레이크포인트 확인
- [x] 글로벌 스타일 (`app/globals.css`)
  - 기본 폰트 설정
  - 다크 모드 변수 (추후 사용)
  - 코드 블록 스타일
...
- [x] `getPosts()` 함수 실행 시 Notion 데이터 정상 반환
- [x] `PostCard` 컴포넌트 스토리북 또는 테스트 페이지에서 확인
- [x] `Layout` 컴포넌트 렌더링 확인
- [x] TypeScript 타입 에러 0개
- [x] ESLint 에러 0개
...
| Phase                            | 상태      | 진행률 | 예상 기간 |
| -------------------------------- | --------- | ------ | --------- |
| Phase 1: 프로젝트 초기 설정      | ✅ 완료   | 100%   | 1주       |
| Phase 2: 공통 모듈/컴포넌트 개발 | ✅ 완료   | 100%   | 1주       |
...
- ✅ Phase 1.1: 개발 환경 구축 (Next.js, TypeScript, ESLint, Prettier, Tailwind CSS, shadcn/ui 설정)
- ✅ Phase 1.2: Notion API 연동 (인티그레이션 생성, API 키 발급, 환경 변수 설정, 패키지 설치)
- ✅ Phase 1.3: Notion 데이터베이스 구성 (데이터베이스 생성, 샘플 데이터 작성, 접근 권한 부여)
- ✅ Phase 1.4: 타입 정의 (types/notion.ts 생성)
- ✅ Phase 2.1: Notion API 유틸리티 함수 개발 (lib/notion.ts)
- ✅ Phase 2.2: 공통 UI 컴포넌트 개발 (PostCard, PostMeta, Layout)
- ✅ Phase 2.3: 유틸리티 함수 개발 (lib/utils.ts)
- ✅ Phase 2.4: 스타일링 기반 구축
